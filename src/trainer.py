...
from .utils import parse_lambda_config, update_lambdas, nanmean
...
class EncDecTrainer(Trainer):
    ...
    def pseudo_ref_mt_step_dec(self, x2, len2, enc1, len1, lang2, y, y_len):
        """
        ...
        alen = torch.arange(y_len.max(), dtype=torch.long, device=y_len.device)
        pred_mask = alen[:, None] < y_len[None] - 1  # do not predict anything given the last target word
        # y = x2[1:].masked_select(pred_mask[:-1])

        # y1 = y[1:].masked_select(pred_mask[:-1])
        y = y[1:].reshape(y_len.max() - 1, self.params.num_hypotheses - 1, -1)
        # assert len(y) == (y_len - 1).sum().item()

        # decode target sentence
        dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
        # dec2.size() = (max_len, (num_hypotheses - 1) * bs, dim)
        dec2 = dec2.reshape(len2.max(), self.params.num_hypotheses - 1, int(len2.size(0) / (self.params.num_hypotheses - 1)), -1)
        # dec2.size() = (max_len, num_hypotheses - 1, bs, dim)

        # y = y.reshape(-1)
        scores, _ = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False, reduction='none', sentence_level=True)

        pred_mask = alen[:, None] >= y_len[None] - 1
        pred_mask = pred_mask.reshape(y_len.max(), self.params.num_hypotheses - 1, -1).unsqueeze(3).expand_as(scores)
        scores.masked_fill_(pred_mask, float('-inf'))
    ...
    def pseudo_ref_mt_step(self, y, y_len, scores):
        """
        Machine translation step.
        Can also be used for denoising auto-encoding.
        """
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        max_l = len1.max().cpu().data.numpy()
        

        generated, gen_lens, costs, generated_scores = self.decoder('autoregressive_train',
                                                                                                  src_enc=enc1,
                                                                                                  src_len=len1,
        ...
        generated_scores = generated_scores.gather(-1, generated.reshape(-1, self.params.num_hypotheses, bs).unsqueeze(3))
        generated_scores = generated_scores.mean(axis=0)
        generated_scores = generated_scores.squeeze(2)

        loss = (generated_scores * costs.cuda()).sum()


        self.stats[('AE-%s' % lang1) if lang1 == lang2 else ('BT-%s-%s-%s' % (lang1, lang2, lang1))].append(loss.item())
